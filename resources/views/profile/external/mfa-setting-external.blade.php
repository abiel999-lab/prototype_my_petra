...
<body class="hold-transition sidebar-mini layout-fixed">
            ...
            <div class="sidebar">
                ...
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                        ...
                        <li class="nav-item">
                            <a href="{{ route('profile.mfa') }}" class="nav-link active">
                                ...
                                <div class="card-header p-2">
                                    <ul class="nav nav-pills">
                                        <li class="nav-item active"><a class="nav-link disabled"
                                                href="#tab_activation" tabindex="-1"
                                                aria-disabled="true">Activation</a></li>
    ...
    <script>
        document.getElementById('passwordless_enabled').addEventListener('change', function() {
            fetch('{{ route('profile.toggle-passwordless') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#extended-mfa-method-form').on('submit', function(e) {
                ...
                $.ajax({
                    url: '{{ route('profile.extended-mfa.setting') }}',
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
